...
public class RightShooting : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(1) && waitTime<=0 && isAvailable)
        {
        ...
        if (Input.GetKey(KeyCode.R) && isReload) cSlider.value = Mathf.Lerp(cSlider.value, 0, cSlider.value * 0.06f * Time.deltaTime);
